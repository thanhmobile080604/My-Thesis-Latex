...
\setlength{\footskip}{1cm} % Đưa footer/số trang lên khỏi mép dưới trang.
\addtolength{\textheight}{-23pt} % Bù phần tăng headheight sau geometry để footer không tràn xuống dưới.
...
\newcommand{\activitydiagram}[3]{%
	\begin{figure}[htbp]
		\centering
		\IfFileExists{#1}{%
			\includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{#1}%
		}{%
...
% Macro rieng cho use case de hinh lon hon cac so do PlantUML thong thuong.
\newcommand{\usecasefig}[3]{%
	\begin{figure}[htbp]
		\centering
		\IfFileExists{#1}{%
