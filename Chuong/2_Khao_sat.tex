...
\subsection{Biểu đồ use case phân rã Video RAFT Server}
\label{subsection:2.2.6}
Use case Video RAFT Server mô tả luồng xử lý video ngoại tuyến bằng máy chủ. Nhóm này được phân rã thành các use case cụ thể trong Bảng~\ref{table:usecase-video-raft}.
...
UC-RFT-02 & Chọn chế độ xử lý & Chọn vector, heatmap hoặc ROI trước khi gửi video. \\ \hline
...
UC-RFT-05 & Tạo job xử lý & Máy chủ tạo \texttt{job\_id} và đưa video vào hàng xử lý nền. \\ \hline
...
UC-RFT-10 & Cleanup tài nguyên server & Gọi endpoint dọn file tạm sau khi tải xong, job lỗi hoặc job bị hủy. \\ \hline
\end{tabular}
\caption{Phân rã use case Video RAFT Server}
...
Về tính tương thích, ứng dụng cần xử lý khác biệt giữa các thiết bị Android về chất lượng GNSS, camera, cảm biến IMU, hiệu năng CPU/GPU và khả năng hỗ trợ thư viện. Những chức năng phụ thuộc phần cứng như dữ liệu GNSS thô, AR hoặc xử lý video cục bộ cần có phương án fallback hoặc thông báo phù hợp khi thiết bị không đáp ứng điều kiện.
